...
usage() {
  cat <<EOF
Usage: $(basename "$0") [-i INPUT_ROOT] [-o OUTPUT_ROOT] [-f] [-q]
...
for daydir in "$INPUT_ROOT"/AE*; do
  ...
  if [ -n "$qc_exists" ] && [ "$FORCE" -eq 0 ]; then
    ...
    DOCKER_CMD=("$AERI_IMG" quality_control.py "$daydir" -o "$outdir" -vv)
    if [ "$FORCE" -eq 1 ]; then
      DOCKER_CMD+=( -f )
    fi
  ...
  if [ -n "$nc_exists" ] && [ "$FORCE" -eq 0 ]; then
    ...
    docker run --rm \
      ...
      -v "$outdir":"$outdir" \
      "$AERI_IMG" \
      dmv_to_netcdf.py "$daydir" -o "$outdir" -vv
  fi
